...
% For the JE scheme, we use the observed SINR values as the convergence indicator
[SINR_JE] = Power_Allocation_JE(K, L, KL, Mr, tau_p, Pt, Noise_power, labels, h_est, Rcorr_error);

% For the SE scheme, we record SINR values and blocklength (max, min, c, d) in each
% iteration as convergence indicators
[Gamma_Meta, Gamma_Payload, N_Meta_c, N_Meta_d, N_Meta_a, N_Meta_b] = ...
   Power_Allocation_SE(Nd, K, L, KL, Mr, tau_p, Package_Meta, Package_Payload, Pt, Noise_power, labels, h_est, Rcorr_error);

% For the SE scheme, we record Z values and max and min values of the BLER
% in each iteration as convergence indicators
[Epsi_max, Epsi_min, Z] = Power_Allocation_RS2_Copy(Nd, R_Meta, R_Payload, K, L, KL, Mr, tau_p, Pt, Noise_power, labels, h_est, Rcorr_error);

save("Convergence.mat", "Epsi_max", "Epsi_min", "Z",... % SE scheme
    "Convergence_JE",... % JE scheme
    "Gamma_Meta", "Gamma_Payload", "N_Meta_c", "N_Meta_d", "N_Meta_a", "N_Meta_b"); % RS scheme
