...
K = 10; % Number of UEs
M_H_ = 8:2:20; % Number of antenna per horizontal row
M_V_ = 8:2:20; % Number of rows
Mr_ = M_V_ .* M_H_; % Number of total antennas
d_H = 1/2; % Horizontal antenna spacing
d_V = 1/2; % Vertical antenna spacing
rs = 50; % Scatter radii
L = 1; % Pilot reuse factor/number of groups
...
for mc = 1:MC
    ...
    for num_ = 1:num
        ...
        NA_SE(num_) = NA_SE(num_) + Power_Allocation_SE(BLER_TH, Nd, K, L, KL, Mr, tau_p, Package_Meta, Package_Payload, Pt, Noise_power, labels, h_est, Rcorr_error);
...
save("Data\NA_VS_Antenna_L1.mat", "NA_JE", "NA_SE", "NA_RS")
