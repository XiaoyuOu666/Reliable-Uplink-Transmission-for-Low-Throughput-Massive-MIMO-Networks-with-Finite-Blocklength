function [akl, ckl] = Detector_Generation_RS(K, L, KL, Mr, Pp, Pm, h_est, labels, Rcorr_error, Noise_power, tau_p, flag)
% Generate MMSE detector and a series of auxiliary variables for the RS scheme
...
for k = 1:K % reconstruct estimation error, power matrix, and channel matrix
    ...
    Prm(k) = Pm(current_group, group_counters(current_group));
    Prp(k) = Pp(current_group, group_counters(current_group));
...
Pm_diag = diag(Prm);
Pp_diag = diag(Prp);
Cr = sum(Rcorr_error_r .* reshape(Prm + Prp, 1, 1, []), 3);

if flag == "Meta"
    V_MMSE = (H_est*P*H_est' + Cr + Noise_power.*eye(Mr)) \ H_est*Pm_diag;
else
    V_MMSE = (H_est*P*H_est' + Cr + Noise_power.*eye(Mr)) \ H_est*Pp_diag;
end
