...
for mc = 1:MC
    ...
    for num_ = 1:num
        NA_SE(num_) = NA_SE(num_) + Power_Allocation_SE(BLER_TH(num_), Nd, K, L, KL, Mr, tau_p, Package_Meta, Package_Payload, Pt, Noise_power, labels, h_est, Rcorr_error);
        NA_JE(num_) = NA_JE(num_) + Power_Allocation_JE(BLER_TH(num_), Nd, R_JE, K, L, KL, Mr, tau_p, Pt, Noise_power, labels, h_est, Rcorr_error);
        NA_RS(num_) = NA_RS(num_) + Power_Allocation_RS(Nd, BLER_TH(num_), R_Meta, R_Payload, K, L, KL, Mr, tau_p, Pt, Noise_power, labels, h_est, Rcorr_error);
    end
    clc
...
save("Data\NA_VS_BLER_TH_MMSE.mat", "NA_JE_MMSE", "NA_SE_MMSE", "NA_RS_MMSE")
