...
BLER_TH = 1E-4; % Different BLER thresholds
PtdBm = 10:4:30; % Transmit power in dBm
...
for mc = 1:MC
    ...
    for num_ = 1:num
        ...
        for kl = 1:tau_p
            for l = 1:L
                y(:, kl) = y(:, kl) + sqrt(Pp(num_)*tau_p)*h_true(:, l, kl);
            end
            y(:, kl) = y(:, kl) + Np;
        end
        ...
        for l = 1:L
            for kl = 1:KL(l)
                for l_pr = 1:L
                    for kl_pr = 1:KL(l)
                        if kl_pr == kl
                            U(:, :, l, kl) = U(:, :, l, kl) + Pp(num_).*tau_p.*Rcorr_r(:, :, l_pr, kl_pr);
                        end
                    end
                end
                U(:, :, l, kl) = U(:, :, l, kl) + Noise_power.*eye(Mr);
                U_inv(:, :, l, kl) = inv(U(:, :, l, kl));
                h_est(:, l, kl) = sqrt(Pp(num_)*tau_p)*Rcorr_r(:, :, l, kl)*U_inv(:, :, l, kl)*y(:, kl);
                Rcorr_est(:, :, l, kl) = Pp(num_).*tau_p.* Rcorr_r(:, :, l, kl) * U_inv(:, :, l, kl) * Rcorr_r(:, :, l, kl);
            end
        end
        ...
        NA_SE(num_) = NA_SE(num_) + Power_Allocation_SE(BLER_TH, Nd, K, L, KL, Mr, tau_p, Package_Meta, Package_Payload, Pt(num_), Noise_power, labels, h_est, Rcorr_error);
        NA_JE(num_) = NA_JE(num_) + Power_Allocation_JE(BLER_TH, Nd, R_JE, K, L, KL, Mr, tau_p, Pt(num_), Noise_power, labels, h_est, Rcorr_error);
        NA_RS(num_) = NA_RS(num_) + Power_Allocation_RS(Nd, BLER_TH, R_Meta, R_Payload, K, L, KL, Mr, tau_p, Pt(num_), Noise_power, labels, h_est, Rcorr_error);
    end
    clc
...
save("Data\NA_VS_POWER_L1.mat", "NA_JE_MMSE", "NA_SE_MMSE", "NA_RS_MMSE")
