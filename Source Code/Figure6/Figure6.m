...
rs = 50; % Scatter radii
L_ = [1, 2]; % Pilot reuse factor/number of groups ()
...
for mc = 1:MC
    ...
    for num_ = 1:num
        ...
        % V_Detector = H_est; % MRC Detector
        V_Detector = H_est / (H_est'*H_est); % ZF Detector
        for kl = 1:K
            V_Detector(:, kl) = V_Detector(:, kl)./norm(V_Detector(:, kl));
        end
        V_r = zeros(Mr, L, tau_p); % Detector after pilot reuse
        for l = 1:L
            group_users = find(labels == l);
            if ~isempty(group_users)
                V_r(:, l, 1:KL(l)) = V_Detector(:, group_users);
            end
        end

        %% Compute auxiliary variables (For ZF and MRC use)
        ...
        for l = 1:L % compute
            for kl = 1:KL(l)
                for l_pr = 1:L
                    for kl_pr = 1:KL(l_pr)
                        akl(l_pr, kl_pr, l, kl) = abs(V_r(:, l, kl)'*h_est(:, l_pr, kl_pr))^2;
                        ckl(l_pr, kl_pr, l, kl) = abs(V_r(:, l, kl)'*Rcorr_error(:, :, l_pr, kl_pr)*V_r(:, l, kl));
                    end
                end
            end
        end
        ...
        R_JE = (Package_Payload + Package_Meta)/Nd; % Coding rate for whole packet in bpcu

        % ZF or MRC
        % NA_SE(num_) = NA_SE(num_) + Power_Allocation_SE_ZF_MRC(BLER_TH, Nd, L, KL, tau_p, Package_Meta, Package_Payload, Pt, Noise_power, akl, ckl);
        % NA_JE(num_) = NA_JE(num_) + Power_Allocation_JE_ZF_MRC(BLER_TH, Nd, R_JC, L, KL, tau_p, Pt, Noise_power, akl, ckl);
        % NA_RS(num_) = NA_RS(num_) + Power_Allocation_RS_ZF_MRC(BLER_TH, Nd, R_Meta, R_Payload, L, KL, tau_p, Pt, Noise_power, akl, ckl);
...
save("Data\NA_MMSE.mat", "NA_JE", "NA_SE", "NA_RS");
